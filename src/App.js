    ...
    <div className="min-h-screen bg-[#020617] text-slate-300 font-sans selection:bg-blue-500/30">
      ...
      <nav className="fixed top-0 w-full z-50 bg-[#020617]/90 backdrop-blur-md border-b border-white/5">
        <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <div className="flex gap-6 items-center uppercase text-[10px] font-bold tracking-widest">
            ...
            <a 
              href="https://linkedin.com/in/dodo-tahirou" 
              target="_blank" 
              rel="noopener noreferrer" 
              className="bg-blue-600 px-4 py-2 rounded-full text-white hover:bg-blue-500 transition-all"
            >
              LinkedIn
            </a>
          </div>
      ...
      <section id="experience" className="max-w-6xl mx-auto px-6 py-20 border-t border-white/5">
        ...
        <div className="bg-white/5 p-8 rounded-[40px] border border-white/5 flex flex-col md:flex-row justify-between items-start gap-6">
          ...
          <ul className="text-sm space-y-3 max-w-md text-slate-400">
      ...
      <section className="bg-white/5 py-20 px-6">
        <div className="max-w-6xl mx-auto grid md:grid-cols-3 gap-12 text-center text-slate-400">
          <div>
            <h4 className="text-white font-black mb-6 uppercase tracking-widest text-sm font-sans">Web & Mobile</h4>
            <p className="font-mono text-xs leading-loose italic underline decoration-blue-500/50 text-slate-300">React, React Native, Node.js, JavaScript, Tailwind</p>
          </div>
          <div>
            <h4 className="text-white font-black mb-6 uppercase tracking-widest text-sm font-sans">IA & ML</h4>
            <p className="font-mono text-xs leading-loose italic underline decoration-blue-500/50 text-slate-300">Machine Learning, CNN, TensorFlow, Python, Data Science</p>
          </div>
          <div>
            <h4 className="text-white font-black mb-6 uppercase tracking-widest text-sm font-sans">Génie Logiciel</h4>
            <p className="font-mono text-xs leading-loose italic underline decoration-blue-500/50 text-slate-300">Java, C# (.NET), C++, SQL, NoSQL (MongoDB)</p>
          </div>
        </div>
      </section>
      ...
      <section id="projets" className="max-w-6xl mx-auto px-6 py-24">
        ...
        <div className="grid md:grid-cols-2 gap-8 text-slate-400">
          <div className="bg-slate-900 p-10 rounded-[40px] border border-white/5 hover:border-blue-500/30 transition-all">
            <h3 className="text-2xl font-black text-white mb-4">Détection de Malwares via CNN</h3>
            <p className="text-blue-400 font-bold text-sm mb-6 italic underline">Précision record : 92%</p>
            <p className="text-sm leading-relaxed">Modèle de Deep Learning haute précision développé pour l'identification automatique de menaces cyber.</p>
          </div>
          <div className="bg-slate-900 p-10 rounded-[40px] border border-white/5 hover:border-blue-500/30 transition-all">
            <h3 className="text-2xl font-black text-white mb-4">Plateforme Médicale .NET</h3>
            <p className="text-slate-200 text-sm mb-6 font-bold">C# .NET | SQL</p>
            <p className="text-sm leading-relaxed">Système complet de prise de rendez-vous pour une clinique médicale, optimisant les flux patients.</p>
          </div>
        </div>
      </section>

      {/* LEADERSHIP */}
      <section className="max-w-6xl mx-auto px-6 py-20 bg-blue-600 rounded-[50px] mb-20 text-center shadow-2xl shadow-blue-500/20">
        ...
        <div className="grid md:grid-cols-2 gap-10 text-white/90">
          <div>
            ...
            <p className="text-sm opacity-80">Association de la Fraternité Africaine à Errachidia (AFAE)</p>
          </div>
          <div>
            <h4 className="font-black text-lg">Secrétaire Général</h4>
            <p className="text-sm opacity-80">Association des Étudiants Nigériens au Maroc (ANEM)</p>
          </div>
        </div>
      </section>
