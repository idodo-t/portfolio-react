...
function App() {
  ...
  const photoPath = "/ma-photo.jpg"; // Vérifie si c'est .jpg ou .png
    ...
    <div className="min-h-screen bg-[#020617] text-slate-300 font-sans selection:bg-blue-500/30 text-left">
        ...
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row items-center gap-12">
          ...
          <div className="w-48 h-48 md:w-72 md:h-72 shrink-0 relative">
            ...
            <img 
              ...
              className="w-full h-full object-cover rounded-[40px] border-2 border-white/10 shadow-2xl relative z-10"
              onError={(e) => { e.target.src = "https://via.placeholder.com/400x400?text=Photo+Profil"; }}
            />
          </div>
      ...
      <section className="bg-[#0f172a]/30 py-16 px-6 border-y border-white/5">
        <div className="max-w-6xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-8">
          ...
          <SkillGroup title="Deep Learning" skills="CNN, TensorFlow, Scikit-learn, Computer Vision, Keras" />
          <SkillGroup title="Full-Stack" skills="React, React Native, Node.js, JavaScript, Java, C#" />
          <SkillGroup title="Data & Cloud" skills="MongoDB, SQL, BI, ETL, Linux, Cybersecurity" />
        </div>
      </section>

      {/* SECTION PROJETS (10 PROJETS) */}
      <section id="projets" className="max-w-6xl mx-auto px-6 py-24">
        <div className="mb-16">
          <h2 className="text-xs font-bold text-blue-500 uppercase tracking-[0.3em] mb-4 text-center md:text-left">Portfolio Technique</h2>
          <h3 className="text-4xl font-black text-white uppercase tracking-tighter italic text-center md:text-left">Réalisations Clés</h3>
        </div>
      ...
      <section id="parcours" className="max-w-6xl mx-auto px-6 py-20 border-t border-white/5">
        <div className="grid md:grid-cols-2 gap-16">
          <div>
            <h3 className="text-xl font-black text-white uppercase mb-8 flex items-center gap-2 underline decoration-blue-500 underline-offset-8">Expérience Pro</h3>
            <div className="bg-white/5 p-8 rounded-[35px] border border-white/5 hover:border-blue-500/20 transition-all group">
              <span className="text-blue-400 font-mono text-xs uppercase tracking-widest">Août – Oct 2024</span>
              <h4 className="text-lg font-black text-white mt-2 group-hover:text-blue-400 transition-colors">Software Engineer</h4>
              <p className="text-slate-500 text-sm mb-6 italic">Technorium Company, Niamey</p>
              <ul className="text-xs space-y-3 text-slate-400">
                <li className="flex gap-3"> <span className="text-blue-500">▹</span> Développement modules Front-end & Back-end (JS & Java).</li>
                <li className="flex gap-3"> <span className="text-blue-500">▹</span> Optimisation de scripts système pour la performance.</li>
                <li className="flex gap-3"> <span className="text-blue-500">▹</span> Collaboration en équipe Agile et déploiement.</li>
              </ul>
            </div>
          </div>
          <div>
            <h3 className="text-xl font-black text-white uppercase mb-8 flex items-center gap-2 underline decoration-blue-500 underline-offset-8">Formation</h3>
      ...
      <section className="max-w-6xl mx-auto px-6 py-20 bg-gradient-to-br from-blue-600 to-blue-800 rounded-[60px] mb-20 text-center shadow-2xl shadow-blue-500/30 mx-4">
        <h2 className="text-white font-black text-2xl mb-12 uppercase tracking-[0.2em] italic">Leadership & Engagement</h2>
        <div className="grid md:grid-cols-3 gap-10 text-white">
          <LeadershipItem title="Président (et VP)" org="Assoc. Fraternité Africaine (AFAE)" period="2021-2024" />
          <LeadershipItem title="Secrétaire Gén." org="Assoc. Étudiants Nigériens (ANEM)" period="2022-2023" />
          <LeadershipItem title="Membre actif" org="Conféd. Étudiants Africains (CESAM)" period="2021-2024" />
        </div>
      </section>

      <footer className="py-12 text-center text-slate-600 text-[10px] font-bold uppercase tracking-[0.5em] border-t border-white/5">
...
function ProjectCard({ icon, title, tech, desc }) {
  return (
    <div className="group bg-slate-900/40 p-8 rounded-[40px] border border-white/5 hover:border-blue-500/40 transition-all duration-500 hover:shadow-2xl hover:shadow-blue-500/10">
      <div className="mb-6 bg-blue-500/10 w-14 h-14 rounded-2xl flex items-center justify-center border border-blue-500/20 group-hover:bg-blue-500 group-hover:text-white transition-all duration-500 text-blue-400">
        {icon}
      </div>
...
function EducationItem({ title, school, date, current }) {
  return (
    <div className={`border-l-2 ${current ? 'border-blue-500' : 'border-slate-800'} pl-6 hover:border-blue-400 transition-colors`}>
      <h4 className="text-white font-bold uppercase text-sm tracking-tight">{title}</h4>
      <p className="text-xs text-slate-500 mt-1">{school} • <span className={current ? 'text-blue-400 font-bold' : ''}>{date}</span></p>
    </div>
  );
}

function LeadershipItem({ title, org, period }) {
  return (
    <div className="group">
      <h4 className="font-black text-xl uppercase leading-tight group-hover:scale-105 transition-transform">{title}</h4>
      <p className="text-[11px] opacity-80 mt-2 uppercase tracking-widest font-bold">{org}</p>
      <p className="text-[10px] opacity-50 mt-1 font-mono">{period}</p>
    </div>
  );
}
